...
def build_surface(rec, alignment, origin) -> tuple:
    ...
    tracks = np.array([len(p.track.elements) for p in rec.points3D.values()])
    cloud = alignment.apply(np.array([p.xyz for p in rec.points3D.values()]))
    cloud = cloud[tracks >= MIN_TRACK_LENGTH]
    ...
    for label, width in ORTHO_WIDTH.items():
        ...
        surface = drape.heightfield(cloud, frame, rect,
                                    posting_m=SURFACE_POSTING_M[label])
...
def surface_payload(frame, packs, label: str) -> dict:
    ...
    return {
        ...
        "uv_float32_base64": b64(surface.uv.astype(np.float32)),
        "support_float32_base64": b64(surface.support_m.astype(np.float32)),
